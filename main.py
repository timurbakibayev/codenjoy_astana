    ...
    for i in range(len(a)):
        for j in range(len(a[i])):
            if a[i][j]["type"] in ["head","body"]:
                ...
                print(a[i][j]["sym"].replace('☼', '#').replace('▲', 
            ...
            for j in range(length):
                if m[i][j] == k:
                    ...
                    if j > 0 and m[i][j-1] == -1 and safe(i,j-1,a,k-1):
                        m[i][j-1] = k+1
                        changes = True
                    if i < len(m)-1 and m[i+1][j] == -1 and 
safe(i+1,j,a,k-1):
                        m[i+1][j] = k+1
                        changes = True
                    if j < len(m[i])-1 and m[i][j+1] == -1 and 
safe(i,j+1,a,k-1):
                        m[i][j+1] = k+1
                        changes = True
        k += 1
...
if __name__ == "__main__":
    ...
    ws = 
websocket.WebSocketApp("ws://codenjoy.astanajug.net:8080/codenjoy-contest/ws?user=timurbakibayev@gmail.com&code=1526207402717100265",
